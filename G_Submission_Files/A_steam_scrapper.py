...
def get_reviews(appid, params={"json": 1}):
    """
    This function is to get the reviews
    from the Steam API.
    """
...
def get_n_reviews(appid, n=100):
    """
    To iterate through the review
    and get the maximum number of revies we can get.
    """
    reviews = []
    cursor = "*"
    params = {
...
x = get_n_reviews(
    "1091500", 237832
)  # the number of reviews for CP2077 as reported by Steam
...
"""This chunk of code is to test the scraper. It's not necessary for the final product."""
...
def reviews_to_df(reviews_dn):
    ...
    for review_set in reviews_dn:
        for review in review_set:
            review_ls.append(
                [review["review"], review["voted_up"], review["timestamp_created"]]
            )
    review_df = pd.DataFrame(
        review_ls,
        columns=["Review", "Recommended or Not Recommended", "Date Timestamp Created"],
    )
...
cp2077_df.to_csv("A_Source_Data/cp2077_reviews.csv.zip", index=False, compression="zip")

# # Checking if the csv is correctly exported
# pd.read_csv('A_Source_Data/cp2077_reviews.csv.zip', compression='zip',nrows=100)
